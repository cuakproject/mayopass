...
app.all('/api', async (req, res) => {
    try {
        ...
        if (action === 'getGamepassData') {
            ...
            return res.json({ success: true, gamepassData: data.gamepassData, resellerData: data.resellerData });
        }
            ...
            if (gamepassData) Object.assign(current.gamepassData, gamepassData);
            if (resellerData) current.resellerData = resellerData;
            ...
            if (hasReseller) {
                ...
                current.resellerData[newName] = { rate: resellerRate, items: items };
            }
            await writeData(current);
...
app.get('/', (req, res) => {
    res.json({ status: 'online', message: 'Mayoblox API is running', timestamp: new Date().toISOString() });
});
